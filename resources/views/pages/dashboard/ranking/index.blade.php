<x-layouts.app-dashboard title="{{ $title }}">
    ...
    <div class="my-8">
        <h4 class="mb-6 text-2xl font-semibold text-gray-900">Hasil Ranking Kinerja Karyawan Tahun Ajaran
            {{ $tahunAjaran }}</h4>
                ...
                <thead class="bg-slate-100 text-base capitalize text-gray-900">
                    <tr>
                        <th class="border-b bg-slate-100 py-2 text-center font-bold text-gray-900"
                            ...
                            Perhitungan Bobot Prioritas Alternatif Terhadap Bobot Prioritas Kriteria
                        </th>
                    </tr>
                    ...
                    @foreach ($alternatifPenilaian as $dataAlternatif)
                        <tr class="border-b bg-white">
                            <th class="w-12 whitespace-nowrap bg-slate-100 px-6 py-4 font-medium text-gray-900"
                                scope="row">
                                {{ $dataAlternatif->alternatifKedua->nama_alternatif }}
                            </th>

                            @foreach ($bobotAlternatif as $bobot)
                                @if ($bobot->kode_alternatif == $dataAlternatif->alternatifKedua->kode_alternatif)
                            ...
                            @foreach ($totalBobotKriteria as $keyAlternatif => $totalBobot)
                                @if ($keyAlternatif == $dataAlternatif->alternatifKedua->kode_alternatif)
                                    <td class="px-3 py-3 text-center">
                                        <input
                                            class="w-24 rounded-md border-none bg-slate-100 text-center focus:ring-slate-100"
            ...
            <table class="w-full text-left text-base text-gray-900">
                <thead class="bg-slate-100 text-base capitalize text-gray-900">
                    <tr>
                        <th class="border-b bg-slate-100 py-2 text-center font-bold text-gray-900"
                            colspan="{{ $kriteria->count() + 2 }}">
                            Rata-rata nilai Alternatif Terhadap Kriteria
                        </th>
                    </tr>
                ...
                <tbody>
                    @foreach ($alternatifPenilaian as $dataAlternatif)
                        <tr class="border-b bg-white">
                            ...
                            @foreach ($avgNilaiKriteria[$dataAlternatif->alternatifKedua->kode_alternatif] as $kodeSubkriteria => $nilai)
                                <td class="px-3 py-3 text-center">
                                    <input
                                        class="w-24 rounded-md border-none bg-slate-100 text-center focus:ring-slate-100"
