    ...
    <div>
        ...
        @foreach ($kriteria as $dataKriteria)
            ...
            <div class="relative my-8 overflow-x-auto rounded-lg shadow-sm">
                <table class="table-alternatif w-full text-left text-base text-gray-900">
                    <thead class="bg-slate-100 text-base capitalize text-gray-900">
                        <p class="border-b bg-slate-100 py-2 text-center font-bold text-gray-900">Perhitungan Bobot
                            Prioritas Alternatif Pada Kriteria : {{ $dataKriteria->nama_kriteria }}
                        </p>
                        ...
                        @foreach ($alternatif as $alternatif1)
                            <tr class="border-b bg-white">
                                ...
                                @foreach ($alternatif as $alternatif2)
                                    <td class="px-3 py-3 text-center">
                                        @php
                                            $normalizedValue = $normalizedMatrix[$dataKriteria->kode_kriteria][$alternatif1->kode_alternatif][$alternatif2->kode_alternatif];
                                        @endphp
                                        ...
                                        <input
                                            ...
                                            value="{{ $normalizedValue
                                                ? $normalizedValue
                                                : $normalizedMatrix[$dataKriteria->kode_kriteria][$alternatif2->kode_alternatif][$alternatif1->kode_alternatif] }}"
                                            @readonly(true)>
                                    </td>
                                @endforeach
                                ...
                                <td class="px-3 py-3">
                                    <input
                                        class="w-20 rounded-md border-none bg-slate-100 text-center focus:ring-slate-100"
                                        type="text"
                                        value="{{ $bobot[$dataKriteria->kode_kriteria][$alternatif1->kode_alternatif] }}"
                                        @readonly(true)>
                            </tr>
                        @endforeach
                    </tbody>
