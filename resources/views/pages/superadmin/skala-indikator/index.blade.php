<x-app-dashboard title="{{ $title }}">
    ...
    <x-molecules.breadcrumb.breadcrumb>
        <li class="xs:text-xs flex items-center gap-0.5 text-gray-800 sm:text-sm dark:text-white/90">
            ...
            <span>Skala Indikator</span>
        </li>
    </x-molecules.breadcrumb.breadcrumb>

    <div class="my-8">
        <div class="flex flex-row items-center justify-end gap-4">
            @can('direct.permission', 'kelola skala indikator')
                @if ($nilaiSkala->isEmpty())
                    <a href="{{ route('nilaiSkala.create') }}">
                        <x-atoms.button.button-primary :type="'button'" :name="'Nilai Skala'" />
                    </a>
                @else
                ...
                <div>
                    <a href="{{ route('skalaIndikator.create') }}">
                        <x-atoms.button.button-primary :type="'button'" :name="'Tambah Skala Indikator'" />
                    </a>
                </div>
            @endcan
        </div>
    ...
    <div
        ...
        <x-molecules.table.table :title="'Daftar Indikator'" :placeholderSearch="'Cari Indikator'" :request="request('indikator_subkriteria')" :nameSearch="'indikator_subkriteria'">
            <x-slot:thead>
                <tr class="border-y border-gray-100 dark:border-gray-800">
                    ...
                    <th class="py-3">
                        <div class="flex items-center">
                            <p class="xs:text-xs sm:text-theme-md font-medium text-gray-800">Nama Indikator</p>
                        </div>
                    </th>
            ...
            @forelse ($skalaIndikator as $item)
                <tr>
                    <td class="py-3">
                        <div class="flex items-center">
                    ...
                    <td class="xs:gap-2.5 flex justify-center p-6 sm:gap-4">
                        @if (Auth::user()->hasAnyDirectPermission(['view skala indikator', 'kelola skala indikator']) ||
                                Auth::user()->canany(['view skala indikator', 'kelola skala indikator']))
                        ...
                        @can('direct.permission', 'kelola skala indikator')
                            ...
                            <div x-data="{ showTooltip: false, isOpen: false }">
                                ...
                                <x-molecules.modal.modal-delete :title="'Apakah Anda yakin ingin menghapus nama indikator: ' .
