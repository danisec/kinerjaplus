<x-app-dashboard title="{{ $title }}">
    ...
    <form class="my-8" action="{{ route('groupPenilaian.update', $groupKaryawan->id_group_karyawan) }}" method="POST">
        @method('PUT')
        @csrf
        <div class="space-y-6">
                                ...
                                <select
                                    ...
                                    @foreach ($groupKaryawanArray as $nestedIndex => $nestedItem)
                                        @if ($nestedItem['kode_alternatif'] != $item['kode_alternatif'])
                                            @php
                                                // Check if it exists in the group_penilaian
                                                $isSelected = false;
                                                foreach ($item['group_penilaian'] as $penilaian) {
                                                    if (
                                                        $penilaian['alternatif_kedua'] == $nestedItem['kode_alternatif']
                                                    ) {
                                                        $isSelected = true;
                                                        break;
                                                    }
